...
public class Principal extends FuseStubFS {
    ...
    private abstract class MemoryPath implements java.io.Serializable {
        ...
        protected void generalAttr(FileStat stat) {
            stat.st_uid.set(this.uId);
            stat.st_gid.set(this.gId);
            stat.st_mtim.tv_sec.set( new java.util.Date().getTime()/1000);
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            principal.mount(Paths.get("/home/euroegas/mnt"), true);
        } finally {
            principal.umount();
        }
    }

    private MemoryDirectory rootDirectory = null;
    ...
    public int readdir(String path, Pointer buf, FuseFillDir filter, @off_t long offset, FuseFileInfo fi) {
    	System.out.println("readir: "+path);
        MemoryPath p = getPath(path);
        if (p == null) {
            return -ErrorCodes.ENOENT();
        }
    ...
    public int write(String path, Pointer buf, @size_t long size, @off_t long offset, FuseFileInfo fi) {
    	System.out.println("write: "+path+" offset: "+offset);
        MemoryPath p = getPath(path);
        if (p == null) {
            return -ErrorCodes.ENOENT();
        }
